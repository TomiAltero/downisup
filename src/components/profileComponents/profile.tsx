...
export default function Profile({ token }: { token: string | null }) {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      if (token) {
        const fetchedUsuario = await getUserProfile({ token });
      ...
      <article className="flex flex-col w-full lg:w-1/2 border-2 border-gray-200 dark:border-gray-700 p-10 rounded-[36px] shadow-lg bg-white dark:bg-gray-900">
        <article className="flex flex-col items-center">
          <Image
            ...
            width={150}
            height={150}
            className="rounded-full border-gray-500 dark:border-gray-400 border-2 pointer-events-none mb-2"
          />
          <h1 className="text-xl text-black dark:text-white font-semibold text-center">
            {nombreCompleto}
          </h1>
          <h3 className="text-base text-gray-400 dark:text-gray-300">
            {usuario.email}
          </h3>
        </article>
        <section className="flex mt-5 border-gray-300 dark:border-gray-600 border-2 rounded-3xl py-3 px-4">
          <article className="w-full flex flex-col items-center justify-center">
            ...
            <h1 className="text-base text-gray-500 dark:text-gray-300">
              {usuario.username}
            </h1>
          </article>
        </section>
        ...
        <section className="flex border-gray-300 dark:border-gray-600 border-2 rounded-3xl py-3 px-4 items-center">
          <article className="w-full flex flex-col items-center justify-center">
            ...
            <h1 className="text-base text-gray-500 dark:text-gray-300 my-1">
              ************
            </h1>
          </article>
        </section>
        <a
          ...
          className="text-base text-center text-custom-blue dark:text-blue-300 mt-5"
        >
          Cambiar Contraseña
        </a>
      </article>
