...
const PopUpNeurologicalTherapies = ({
  onClose,
  hijoId,
}: {
  onClose: () => void;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          data &&
          data.neurologicalTherapies &&
          data.neurologicalTherapies.length > 0
        ) {
          setSessionData(data);
        } else {
          setSessionData({
            ...data,
            neurologicalTherapies: [],
          });
        }
      } catch (error) {
    ...
    >
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-6 text-blue-900">
          Informe de Sesión Neurológica:{" "}
          {hijo?.nombre || "Nombre del paciente no disponible"}{" "}
          {hijo?.apellido || ""}
        </h2>

        {sessionData?.neurologicalTherapies?.length > 0 ? (
          sessionData.neurologicalTherapies.map(
            (therapy: any, index: number) => (
              <div key={index} className="mt-6 space-y-6 text-left">
                ...
                {index < sessionData.neurologicalTherapies.length - 1 && (
                  <hr className="my-6" />
                )}
              </div>
            ),
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
            No hay terapias neurológicas disponibles.
          </p>
        )}
      </motion.div>
    </motion.div>
