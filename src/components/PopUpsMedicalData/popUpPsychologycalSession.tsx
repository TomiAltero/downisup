...
const PopUpPsychologycalSession = ({
  onClose,
  hijoId,
}: {
  onClose: () => void;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          data &&
          data.psychologicalTherapies &&
          data.psychologicalTherapies.length > 0
        ) {
          setSessionData(data);
        } else {
          setSessionData({
          ...
          sessionData.psychologicalTherapies.map(
            (therapy: any, index: number) => (
              <div key={index} className="mt-6 space-y-6 text-left">
                ...
                {index < sessionData.psychologicalTherapies.length - 1 && (
                  <hr className="my-6" />
                )}
              </div>
            ),
