...
const PhysiologicalSessionForm = ({ hijoId }: { hijoId: string }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://downisup-api-production.up.railway.app/api/medicalData/physiologicalTherapies', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        Toastify({
          text: "Sesión fisiológica agregada exitosamente.",
          duration: 5000,
          position: "right",
          style: {
            background: "#009933",
        ...
        console.log('Sesión fisiológica agregada:', result);
        resetForm();
      } else {
        Toastify({
          text: "Error al agregar la sesión fisiológica.",
          duration: 5000,
          position: "right",
          style: {
            background: "#FF0000",
        ...
        console.error('Error al agregar la sesión fisiológica:', result);
      }
    } catch (error) {
      Toastify({
  ...
  return (
    <div className="bg-gray-100 p-10 rounded-lg shadow-lg w-full lg:w-3/4 xl:w-2/3 mx-auto">
      <h2 className="text-2xl font-bold text-blue-900 mb-4 text-center">Formulario Sesión Fisiológica</h2>
