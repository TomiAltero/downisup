...
export function SignUp() {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Error registrando el usuario:", error.response?.data || error.message);
      if (error.response && error.response.data && error.response.data.errors) {
        const validationErrors = error.response.data.errors.map((error: any) => error.msg);
    ...
    <form onSubmit={handleFormSubmit}>
      <Card className="mt-8 w-[450px]">
        <CardHeader className="flex flex-col items-center -mt-3">
          ...
          <CardTitle className="-mt-2 my-2 text-xl font-bold text-blue-900">Regístrate</CardTitle>
        </CardHeader>
